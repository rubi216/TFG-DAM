...
app.post('/modifyCar/:id', verificarToken, upload.single('foto'), (req, res) => {
    ...
    const foto = req.file?.filename;
    const inputs = {};
    if (req.body.matricula) {inputs.matricula = req.body.matricula};
    inputs.matricula = inputs.matricula.toUpperCase();
    if (req.body.marca) {inputs.marca = req.body.marca};
    inputs.marca = inputs.marca.charAt(0).toUpperCase() + inputs.marca.slice(1); 
    if (req.body.modelo) {inputs.modelo = req.body.modelo};
